...
<div class="text-center">
    <a class="btn btn-secondary mt-3" href="resultados_relevo4x50metros_cb.php" style="background-color: blue; border-color: blue; color: white;">Agregar resultados a resultados_relevo4x50metros_cb</a>
</div>
<div class='text-center mt-3'>
    <a href='javascript:history.go(-1)' class='btn btn-secondary btn-custom' style="background-color: orange; border-color: orange; color: white;">Volver Atrás</a>
</div>
<br><br>
...
function mostrarFormularioSeleccion($conn, $mensaje = "") {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_row()) {
            $tabla = $row[0];
            if (in_array(substr($tabla, -2), ['cb']) && $tabla !== 'resultados_relevo5x80metros_cb') {
